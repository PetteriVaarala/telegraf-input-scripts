...
def get_snapraid_status():
    status_output = subprocess.Popen(['sudo', 'snapraid', 'status', '-v'],
...
def parse_memory_for_filesystem(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array memory_use={0} {1}'\
              .format(memory_in_bytes,
                      unix_timestamp)
...
def parse_not_scrubbed(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array not_scrubbed_percent={0} {1}'\
              .format(not_scrubbed_percent,
                      unix_timestamp)
...
def parse_subsecond_timestamp(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array '\
              'subsecond_timestamp_files={0} {1}'\
              .format(subsecond_timestamp_files,
                      unix_timestamp)
...
def parse_hardlinks(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array hardlinks={0} {1}'\
              .format(hardlinks,
                      unix_timestamp)
    return hardlinks
...
def parse_symlinks(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array symlinks={0} {1}'\
              .format(symlinks,
                      unix_timestamp)
    return symlinks
...
def parse_empty_dirs(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array empty_dirs={0} {1}'\
              .format(empty_dirs,
                      unix_timestamp)
    return empty_dirs
...
def parse_scrubs(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array scrub_oldest={0},scrub_median={1},'\
              'scrub_newest={2} {3}'\
              .format(scrub_oldest,
                      scrub_median,
                      scrub_newest,
...
def parse_status_report_disk(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Disk,DiskName={0} files={1},'\
              'frag_files={2},excess_frag={3},wasted_gb={4},used_gb={5},'\
              'free_gb={6},use_percentage={7} {8}'\
              .format(disk_stats["name"],
                      disk_stats["files"],
...
def parse_status_report_total(line):
    ...
    if args.influx:
        print 'snapraid_status,Type=Array files={0},frag_files={1},'\
              'excess_frag={2},wasted_gb={3},used_gb={4},free_gb={5},'\
              'use_percentage={6} {7}'\
              .format(total_disk_stats["total_files"],
...
snapraid_status_output = get_snapraid_status()
...
for line in snapraid_status_output:
    ...
    if match("^\s+\d+\s+\d+\s+\d+\s+-?\d+.\d+\s+-?\d+\s+-?\d+\s+\d+%+\s+\w+",
             line):
...
if args.debug:
    ...
    print 'Files with zero sub-second timestamp: {0}'\
          .format(subsecond_timestamps)
